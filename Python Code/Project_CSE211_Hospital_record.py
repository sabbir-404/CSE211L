class PatientRecordSorter:
    def __init__(self, arr):
        self.patient_records = arr

    def count_sort_numbers(self, exp, index):
        arr_len = len(self.patient_records)
        output = [0] * arr_len
        arr_count = [0] * 10

        for i in range(arr_len):
            arr_index = self.patient_records[i][index] // exp
        ...
        while i >= 0:
            arr_index = self.patient_records[i][index] // exp
            output[arr_count[arr_index % 10] - 1] = self.patient_records[i]
            arr_count[arr_index % 10] -= 1
            i -= 1

        for i in range(arr_len):
            self.patient_records[i] = output[i]

    def radix_sort(self, index):
        max_value = max([data[index] for data in self.patient_records])
        exp = 1
        
        while max_value // exp > 0:
            self.count_sort_numbers(exp, index)
            exp *= 10
